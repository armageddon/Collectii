  ...
  module ClassMethods
    ...
    def paginating_find(*args)
      ...
      if page_options
        ...
        count = sum_grouped_count(count(collect_count_options(options))) unless count
    ...
    def sum_grouped_count(ordered_hash)
      unless ordered_hash.respond_to? 'flatten'
        ordered_hash
      else   
        Hash[*ordered_hash.flatten].values.inject(0) { |sum, val| sum = sum + val }
      end
    end
    
    def collect_count_options(options)
      # AR::Base#find does not support :having, so I do not support it here either.
      valid_options = [:conditions, :joins, :order, :group, :distinct, :limit, :offset, :include]
      rtn = {}.merge(options)
      if rtn[:select]
